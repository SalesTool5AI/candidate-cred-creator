...
function AnimatedCounter({ target, suffix }: { target: number | string; suffix: string }) {
  ...
  return (
    <span className="text-2xl sm:text-4xl md:text-5xl font-bold text-primary">
...
export function Achievements() {
  return (
    <section id="achievements" className="py-12 sm:py-20 bg-gradient-to-b from-background to-secondary relative overflow-hidden">
      {/* Background network elements */}
      <div className="absolute inset-0">
        <div className="absolute top-10 left-10 w-px h-20 bg-gradient-to-b from-primary/20 to-transparent transform rotate-12"></div>
        <div className="absolute bottom-20 right-20 w-px h-24 bg-gradient-to-b from-brand-blue/15 to-transparent transform -rotate-12"></div>
        <div className="absolute top-1/2 left-1/4 w-1 h-1 bg-primary/60 rounded-full animate-pulse delay-300"></div>
        <div className="absolute top-1/3 right-1/3 w-1 h-1 bg-brand-blue/60 rounded-full animate-pulse delay-700"></div>
      </div>
